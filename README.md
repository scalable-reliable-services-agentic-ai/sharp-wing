...
2. Probabilistic Layer: Machine Learning-driven anomaly detection (XGBoost) enriched with behavioral user profiling via Redis.
3. Agentic Layer: Deep contextual investigation via Model Context Protocol (MCP). A Triage Agent and a Judge Agent collaborate to resolve complex "Grey Zone" anomalies, providing human-readable reasoning (Explainable AI).
...
* K8s Local Engine: kind (recommended if not using Docker Desktop's internal K8s engine)
